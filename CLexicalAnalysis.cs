    ...
    class CLexicalAnalysis
    {
        ...
        public void BeginAnalysis(RichTextBox codeBox)
        {
            ...
            while (Code[index] != '\0')
            {
                ...
                if (CUtility.IsInvalidChars(charTemp))
                {
                    this.isWordEnd = true;
                }
                //回车符
                ...
                #region 数字
                else if (CUtility.IsDigit(charTemp))
                {
                    String temp = charTemp.ToString();
                    do
                    {
                        charTemp = Code[++index];
                        if (CUtility.IsDigit(charTemp) || CUtility.IsLetter(charTemp) || (charTemp == '.'))
                            temp += charTemp;
                        else
                            index--;

                    } while (CUtility.IsDigit(charTemp) || CUtility.IsLetter(charTemp) || (charTemp == '.'));

                    if (CUtility.IsBiInteger(temp))
                    {
                        MyLexicalToken token = new MyLexicalToken();
                        token.Type = ElementType.BiInteger;
                        token.Name = temp;
                        token.LineCount = lineCount;
                        token.Code = CUtility.BiNum;
                        this.MyTokenList.Add(token);

                    }
                    else if (CUtility.IsOctInteger(temp))
                    {
                        MyLexicalToken token = new MyLexicalToken();
                        token.Type = ElementType.OctInteger;
                        token.Name = temp;
                        token.LineCount = lineCount;
                        token.Code = CUtility.OctNum;
                        this.MyTokenList.Add(token);
                    }
                    else if (CUtility.IsFloat(temp))
                    {
                        MyLexicalToken token = new MyLexicalToken();
                        token.Type = ElementType.Float;
                        token.Name = temp;
                        token.LineCount = lineCount;
                        token.Code = CUtility.FloatNum;
                        this.MyTokenList.Add(token);
                    }
                    else if (CUtility.IsHexInteger(temp))
                    {
                        MyLexicalToken token = new MyLexicalToken();
                        token.Type = ElementType.HexInteger;
                        token.Name = temp;
                        token.LineCount = lineCount;
                        token.Code = CUtility.HexNum;
                        this.MyTokenList.Add(token);
                    }
                    else if (CUtility.IsDecInteger(temp))
                    {
                        MyLexicalToken token = new MyLexicalToken();
                        token.Type = ElementType.DecInteger;
                        token.Name = temp;
                        token.LineCount = lineCount;
                        token.Code = CUtility.DecNum;
                        this.MyTokenList.Add(token);
                    }
                    else
                    {
                ...
                #region 字母
                else if (CUtility.IsLetter(charTemp))
                {
                    string temp = charTemp.ToString();
                    do
                    {
                        charTemp = Code[++index];
                        if (CUtility.IsDigit(charTemp) || CUtility.IsLetter(charTemp))
                            temp = temp + charTemp;
                        else
                            index--;
                    } while (CUtility.IsLetter(charTemp) || CUtility.IsDigit(charTemp));
                    ...
                    if (CUtility.IsKeyWord(temp))
                    {
                        ...
                        token.Code = CUtility.KeyWordBase + CUtility.GetIndexKeyWord(temp);
                    }
                    //是标识符
                    else
                    {
                        token.Type = ElementType.Identifier;
                        token.Code = CUtility.IdBase;
                ...
                else if (charTemp == '\"')
                {
                    ...
                    if (isAbnormal == false)
                    {
                        ...
                        token.Name = temp;
                        token.LineCount = lineCount;
                        token.Code = CUtility.StringCode;
                        this.MyTokenList.Add(token);
                    }
                    //有错误，加入ErrorList
                    else
                ...
                #region 分隔符
                else if (CUtility.IsSeparatorChar(charTemp))
                {
                    if (charTemp == ';')
                    {
                    ...
                    token.LineCount = lineCount;
                    token.Code = CUtility.SeparatorBase + CUtility.GetIndexSeparator(charTemp);
                    this.MyTokenList.Add(token);
                }
                #endregion
                ...
                else if (charTemp == '+' || charTemp == '-' || charTemp == '&' || charTemp == '|')
                {
                    ...
                    token.Name = temp;
                    token.LineCount = lineCount;
                    token.Code = CUtility.OperatorBase + CUtility.GetIndexOperator(charTemp);
                    this.MyTokenList.Add(token);
                }
                #endregion
